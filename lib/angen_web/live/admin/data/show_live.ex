defmodule AngenWeb.Admin.Data.ShowLive do
  @moduledoc false
  ...
  def handle_event("delete-user-token", %{"token_id" => token_id}, socket) do
    token = Angen.Data.get_user_token(token_id)
    Angen.Data.delete_user_token(token)
  ...
  defp get_user(%{assigns: %{user_id: user_id}} = socket) do
    user =
      try do
        Teiserver.Data.get_user(user_id, preload: [:smurf_of])
      rescue
