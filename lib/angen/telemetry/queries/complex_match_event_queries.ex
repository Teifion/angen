defmodule Angen.Telemetry.ComplexMatchEventQueries do
  @moduledoc false
  use TeiserverMacros, :queries
  alias Angen.Telemetry.ComplexMatchEvent
  require Logger

  @spec complex_match_event_query(Teiserver.query_args()) :: Ecto.Query.t()
  def complex_match_event_query(args) do
  ...
  def _where(query, :id, id) do
    from(complex_match_events in query,
      where: complex_match_events.id in ^List.wrap(id)
    )
  end

  def _where(query, :type_id, type_id) do
    from(complex_match_events in query,
      where: complex_match_events.type_id in ^List.wrap(type_id)
    )
  end

  def _where(query, :user_id, user_id) do
    from(complex_match_events in query,
      where: complex_match_events.user_id in ^List.wrap(user_id)
    )
  end

  def _where(query, :match_id, match_id) do
    from(complex_match_events in query,
      where: complex_match_events.match_id in ^List.wrap(match_id)
    )
  end

  def _where(query, :after, timestamp) do
    from complex_match_events in query,
      where: complex_match_events.inserted_at > ^Timex.to_datetime(timestamp)
  end
  ...
  def _order_by(query, "Newest first") do
    from(complex_match_events in query,
      order_by: [desc: complex_match_events.inserted_at]
    )
  end
  ...
  def complex_match_events_summary(args) do
    query =
