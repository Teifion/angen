defmodule Angen.Telemetry.ComplexAnonEventQueries do
  @moduledoc false
  use TeiserverMacros, :queries
  alias Angen.Telemetry.ComplexAnonEvent
  require Logger
  ...
  def _where(query, :id, id) do
    from(complex_anon_events in query,
      where: complex_anon_events.id in ^List.wrap(id)
    )
  end

  def _where(query, :event_type_id, event_type_id) do
    from(complex_anon_events in query,
      where: complex_anon_events.event_type_id in ^List.wrap(event_type_id)
    )
  end

  def _where(query, :hash_id, hash_id) do
    from(complex_anon_events in query,
      where: complex_anon_events.hash_id in ^List.wrap(hash_id)
    )
  end
  ...
  def _order_by(query, "Newest first") do
    from(complex_anon_events in query,
      order_by: [desc: complex_anon_events.inserted_at]
    )
  end
  ...
  def complex_anon_events_summary(args) do
    query =
