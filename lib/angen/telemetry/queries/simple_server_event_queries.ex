defmodule Angen.Telemetry.SimpleServerEventQueries do
  @moduledoc false
  use TeiserverMacros, :queries
  alias Angen.Telemetry.SimpleServerEvent
  require Logger
  ...
  def _where(query, :id, id) do
    from(simple_server_events in query,
      where: simple_server_events.id in ^List.wrap(id)
    )
  end

  def _where(query, :event_type_id, event_type_id) do
    from(simple_server_events in query,
      where: simple_server_events.event_type_id in ^List.wrap(event_type_id)
    )
  end

  def _where(query, :user_id, user_id) do
    from(simple_server_events in query,
      where: simple_server_events.user_id in ^List.wrap(user_id)
    )
  end

  def _where(query, :after, timestamp) do
    from simple_server_events in query,
      where: simple_server_events.inserted_at > ^Timex.to_datetime(timestamp)
  end
  ...
  def simple_server_events_summary(args) do
    query =
