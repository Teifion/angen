defmodule Angen.TextProtocol.CommunicationTest do
  ...
  describe "send_direct_message" do
    test "unauth" do
      ...
      speak(socket, %{name: "communication/send_direct_message", command: %{to_id: "123", content: "123"}})
      msg = listen(socket)
    ...
    test "auth" do
      ...
      # Bad target first
      speak(socket1, %{name: "communication/send_direct_message", command: %{to_id: Ecto.UUID.generate(), content: ""}})
      msg = listen(socket1)
      ...
      # Now good ID
      speak(socket1, %{name: "communication/send_direct_message", command: %{to_id: user2.id, content: "Test message"}})
      msg = listen(socket1)
